...
SkenaHub.Settings = SkenaHub.Settings or { ToggleKey = Enum.KeyCode.Z }
...
SkenaHub.Core.Serialize = function(val, depth)
    depth = depth or 1
    local t = typeof(val)
    if t == "table" then
        ...
        for k, v in pairs(val) do
            ...
            res = res .. tostring(k) .. "=" .. SkenaHub.Core.Serialize(v, depth - 1)
            first = false
        end
        return res .. "}"
...
function SkenaTopNav:CreateWindow(titleText)
    ...
    function Window:CreateTab(tabName, iconId, tabColor, activeIconId)
        ...
        function Elements:CreateDropdown(name, options, callback, parent, dropdownType)
            local ParentFrame = parent or TabPage
            dropdownType = dropdownType or "default" -- default, multi, search, search_multi
            
            local isMulti = dropdownType:find("multi")
            local isSearch = dropdownType:find("search")
            ...
            DropFrame.BackgroundTransparency = (dropdownType == "default") and 0.3 or 0
            ...
            Arrow.Image = "rbxassetid://10709795175" -- chevron-down
            Arrow.ImageColor3 = Theme.TextSubtle
            Arrow.Parent = DropFrame

            -- // Expanded Content Container // --
            local ExpandContainer = Instance.new("Frame")
            ExpandContainer.Name = "DropdownContainer"
            ExpandContainer.Size = UDim2.new(1, 0, 0, 0)
            ExpandContainer.Position = UDim2.new(0, 0, 1, 4)
            ExpandContainer.BackgroundColor3 = Theme.TopBarColor
            ExpandContainer.BackgroundTransparency = 0.1
            ExpandContainer.BorderSizePixel = 0
            ExpandContainer.Visible = false
            ExpandContainer.ClipsDescendants = true
            ExpandContainer.ZIndex = 50
            ExpandContainer.Parent = DropFrame
            Instance.new("UICorner", ExpandContainer).CornerRadius = Theme.InnerCorner
            local ExpandStroke = Instance.new("UIStroke", ExpandContainer)
            ExpandStroke.Color = Theme.BorderGlow
            ExpandStroke.Transparency = 0.5
            ...
            if isSearch then
                ...
                SearchFrame.Position = UDim2.new(0, 6, 0, 8)
                SearchFrame.BackgroundColor3 = Theme.ElementColor
                SearchFrame.Parent = ExpandContainer
                Instance.new("UICorner", SearchFrame).CornerRadius = UDim.new(0, 4)
                
                SearchBox = Instance.new("TextBox")
                SearchBox.Size = UDim2.new(1, (dropdownType == "search_multi" and -60 or -10), 1, 0)
                ...
                SearchBox.FontFace = Theme.Fonts.SemiBold
                SearchBox.TextSize = 11
                SearchBox.TextXAlignment = Enum.TextXAlignment.Left
                SearchBox.Parent = SearchFrame

                if dropdownType == "search_multi" then
                    local BtnSize = 15 -- Square as per request
                    local ControlContainer = Instance.new("Frame")
                    ControlContainer.Size = UDim2.new(0, (BtnSize * 2) + 9, 0, BtnSize)
                    ControlContainer.Position = UDim2.new(1, -( (BtnSize * 2) + 9 + 5 ), 0.5, -BtnSize/2)
                    ControlContainer.BackgroundTransparency = 1
                    ControlContainer.Parent = SearchFrame

                    local function createControlBtn(icon, color, offset)
                        local b = Instance.new("ImageButton")
                        b.Size = UDim2.new(0, BtnSize, 0, BtnSize)
                        b.Position = UDim2.new(0, offset, 0, 0)
                        b.BackgroundTransparency = 1
                        b.Image = icon
                        b.ImageColor3 = Theme.TextSubtle
                        b.Parent = ControlContainer
                        return b
                    end

                    local SelectAll = createControlBtn("rbxassetid://10734884548", Theme.StatusGreen, 0)
                    local DeselectAll = createControlBtn("rbxassetid://10723433655", Theme.StatusRed, BtnSize + 9)

                    SelectAll.MouseButton1Click:Connect(function()
                        selectedOptions = {}
                        for _, opt in pairs(optionButtons) do
                            table.insert(selectedOptions, opt.Name)
                            TweenService:Create(opt, TweenInfo.new(0.2), {BackgroundColor3 = Theme.StatusGreen, BackgroundTransparency = 0.8}):Play()
                            opt.TextColor3 = Theme.StatusGreen
                        end
                        callback(selectedOptions)
                        Label.Text = #selectedOptions .. " Selected"
                    end)

                    DeselectAll.MouseButton1Click:Connect(function()
                        selectedOptions = {}
                        for _, opt in pairs(optionButtons) do
                            TweenService:Create(opt, TweenInfo.new(0.2), {BackgroundColor3 = Theme.TopBarColor, BackgroundTransparency = 0.5}):Play()
            ...
            local listTopOffset = isSearch and 42 or 6
            Content.Size = UDim2.new(1, 0, 1, -listTopOffset - 6)
            Content.Position = UDim2.new(0, 0, 0, listTopOffset)
            Content.BackgroundTransparency = 1
            Content.ScrollBarThickness = 2
            Content.ScrollBarImageColor3 = Theme.AccentRed
            Content.Parent = ExpandContainer
            ...
            local function toggle(state)
                ...
                if expanded then
                    ...
                    DropFrame.ZIndex = 100 -- Ensure it's on top of other elements
                end
                
                local targetSize = expanded and 160 or 0
                TweenService:Create(ExpandContainer, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {Size = UDim2.new(1, 0, 0, targetSize)}):Play()
            ...
            function drop:AddItem(txt)
                ...
                optionButtons[txt] = Opt

                Opt.MouseButton1Click:Connect(function()
                    if isMulti then
                        local found = table.find(selectedOptions, txt)
                        if found then
                        ...
                        Label.Text = #selectedOptions > 0 and (#selectedOptions .. " Selected") or name
                        callback(selectedOptions)
                    else
                        Label.Text = txt
                        callback(txt)
                ...
                Opt.MouseEnter:Connect(function() 
                    if not table.find(selectedOptions, txt) then
                        TweenService:Create(Opt, TweenInfo.new(0.2), {BackgroundTransparency = 0, TextColor3 = Theme.TextPrimary}):Play() 
                    end
                end)
                Opt.MouseLeave:Connect(function() 
                    if not table.find(selectedOptions, txt) then
                        TweenService:Create(Opt, TweenInfo.new(0.2), {BackgroundTransparency = 0.5, TextColor3 = Theme.TextMuted}):Play() 
                    end
                end)
...
local function InitializeFinalTabs()
    ...
    SettingsElements:CreateTextBox("Z", "Toggle UI Key", "Key...", function(val)
        ...
        if success then
            SkenaHub.Settings.ToggleKey = key
...
task.spawn(function()
    task.wait(0.5)
    if Window and Window.Toggle and not isPanelOpen then
        Window:Toggle()
    end
end)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    local key = (SkenaHub.Settings and SkenaHub.Settings.ToggleKey) or Enum.KeyCode.Z
